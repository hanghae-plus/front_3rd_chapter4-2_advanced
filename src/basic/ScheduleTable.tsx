...
import { ComponentProps, Fragment, memo, useCallback, useRef, useState } from "react";
...
const ScheduleTable = ({ tableId, schedules, onScheduleTimeClick, onDeleteButtonClick }: Props) => {
  const { active, setNodeRef } = useDndContext();
  const [activeSchedule, setActiveSchedule] = useState<Schedule | null>(null);
  console.log(schedules,active,tableId)
  ...
  // 삭제 핸들러 메모이제이션
  const memoizedDeleteHandler = useCallback((timeInfo: { day: string, time: number }) => {
  ...
  return (
      <Box
        position="relative"
        outline={active && String(active.id).split(":")[0] === tableId? "5px dashed" : undefined}
      ...
      >
        ...
        {schedules.map((schedule, index) => (
          <DraggableSchedule
            ...
            onDeleteButtonClick={() => memoizedDeleteHandler({
              day: schedule.day,
