...
const fetchAllLectures = (() => {
  ...
  return async () => {
    ...
    console.log('Start fetchAllLectures', startTime);

    const promises = [];

    if (!cachedMajors) {
      console.log('fetchMajors: ', performance.now());
      cachedMajors = fetchMajors();
    } else {
      console.log('cached fetchMajors: ', performance.now());
    }

    if (!cachedLiberalArts) {
      console.log('fetchLiberalArts: ', performance.now());
      cachedLiberalArts = fetchLiberalArts();
    } else {
      console.log('cached fetchLiberalArts: ', performance.now());
    }

    promises.push(cachedMajors);
    promises.push(cachedLiberalArts);
    promises.push(cachedMajors);
    promises.push(cachedLiberalArts);
    promises.push(cachedMajors);
    promises.push(cachedLiberalArts);
    ...
    console.log('finish fetchAllLectures: ', endTime, 'Duration:', endTime - startTime, 'ms');
...
const SearchDialog: React.FC<Props> = ({ searchInfo, onClose }) => {
              ...
              <HStack spacing={4}>
                <FormControl>
                  ...
                  >
                    <HStack spacing={4}>
                      {[1, 2, 3, 4].map(grade => (
                          <Checkbox key={grade} value={grade.toString()}>{grade}학년</Checkbox>
                      ))}
                    </HStack>
                  </CheckboxGroup>
                </FormControl>
                  ...
                  >
                    ...
                    <Stack spacing={2} overflowY="auto" h="100px" border="1px solid" borderColor="gray.200"
                      ...
                      {TIME_SLOTS.map(({ id, label }) => (
                          <Box key={id}>
                            <Checkbox size="sm" value={id.toString()}>
                              {id}교시({label})
                            </Checkbox>
                          </Box>
                      ))}
