...
type MemoizeFunction = <Args extends unknown[], Result>(
  fn: (...args: Args) => Promise<Result>
) => (...args: Args) => Promise<Result>;

const memoize: MemoizeFunction = (fn) => {
  const cache = new Map<string, unknown>();

  return async (...args) => {
    const key = JSON.stringify(args);
    if (cache.has(key)) {
      return cache.get(key) as Awaited<ReturnType<typeof fn>>;
    }
...
// Memoized versions
const memorizedFetchMajors = memoize(fetchMajors);
const memorizedFetchLiberalArts = memoize(fetchLiberalArts);
